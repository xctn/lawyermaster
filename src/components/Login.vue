...
<script>
	import {login, getAdminInfo, register} from '@/api/getData'
		...
		methods: {
			...
			async signIn(formName) {
				...
				this.$refs[formName].validate(async (valid) => {
					...
					if (valid) {
						const res = await register({username: this.regform._username, password: this.regform._password, lawyername: this.regform.lawyername, type: this.regform.type, tel: this.regform.tel, idcard: this.regform.idcard, age: this.regform.age, position: this.regform.position, sex: this.regform.sex})
						if (res.status == 1) {
